...
export default function FarmersAcademyPortal({ onBack, onBuyAgrochemicals }: FarmersAcademyPortalProps) {
  const { workshops: WORKSHOPS, academyCategories: ACADEMY_CATEGORIES } = useContent();
  const priceOf = (l: AcademyLesson) => l.price;
  const videoOf = (l: AcademyLesson) => l.videoUrl || SAMPLE_VIDEO_URL;
  ...
  const ActiveCategoryIcon = (activeCategory ? ACADEMY_ICON_MAP[activeCategory.iconKey] : null) ?? Home;
  ...
  const isLessonUnlocked = (l: AcademyLesson) =>
    priceOf(l) === 0 || unlockedLessons.includes(l.id);
                  ...
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    ...
                    {priceOf(activeLesson) === 0 ? (
                      ...
                      <span className="inline-flex items-center space-x-1 text-[10px] font-bold uppercase tracking-wider text-amber-800 bg-amber-100 px-2.5 py-1 rounded-full">
                        ...
                        <span>Paid · Ksh {priceOf(activeLesson)}</span>
                      </span>
                    )}
                  </div>
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="space-y-0.5">
                          ...
                          <p className="text-xs text-slate-600 leading-relaxed">
                            ...
                            <strong className="text-emerald-800">Ksh {priceOf(activeLesson)}</strong> via M-Pesa.
                          </p>
                        </div>
                      </div>
                      ...
                      >
                        {payingLessonId === activeLesson.id ? (
                          ...
                          <>
                            ...
                            <span>Pay Ksh {priceOf(activeLesson)} with M-Pesa</span>
                          </>
                        )}
                      </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {activeCategory.lessons.map((lesson, idx) => (
                  ...
                  >
                    ...
                    ) : priceOf(lesson) === 0 || isLessonUnlocked(lesson) ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {ACADEMY_CATEGORIES.map((cat) => {
                  const Icon = ACADEMY_ICON_MAP[cat.iconKey] ?? Home;
